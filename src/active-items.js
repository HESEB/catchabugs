...
const ITEM_EFFECTS = Object.freeze({
  'bug-lure': { icon: '🪤', name: '벌레 유인기', durationMin: 30, desc: '곤충 생성량 +50%', spawnNow: 2, stats: { spawnBoost: 0.5 } },
  'rare-alarm': { icon: '🔔', name: '희귀 알람기', durationMin: 30, desc: '희귀 이상 신호 알림', spawnNow: 1, stats: { rareAlarm: 1, rareBoost: 0.5 } },
  'radar-boost': { icon: '📡', name: '레이더 증폭기', durationMin: 30, desc: '레이더 거리 +20m', spawnNow: 1, stats: { radarBonus: 20 } },
  'sap-trap': { icon: '🪵', name: '수액통', durationMin: 45, desc: '숲 곤충 유인 +30%', spawnNow: 2, stats: { forestBoost: 0.3, spawnBoost: 0.2 } },
  'flower-lure': { icon: '🌼', name: '꽃 유인기', durationMin: 45, desc: '초원 곤충 유인 +30%', spawnNow: 2, stats: { fieldBoost: 0.3, spawnBoost: 0.2 } },
  'water-trap': { icon: '💧', name: '수생 트랩', durationMin: 45, desc: '강가 곤충 유인 +30%', spawnNow: 2, stats: { riverBoost: 0.3, spawnBoost: 0.2 } },
  'bug-light': { icon: '💡', name: '곤충등', durationMin: 45, desc: '야행성 신호 강화', spawnNow: 2, stats: { nightBoost: 0.3, rareBoost: 0.3 } },
  'bug-cookie': { icon: '🍪', name: '곤충 쿠키', durationMin: 15, desc: '일반 곤충 출현 +20%', spawnNow: 1, stats: { spawnBoost: 0.2 } },
});
...
function cleanState(state = loadRaw()) {
  const t = now();
  const active = (state.active || []).filter((item) => Number(item.endsAt || 0) > t);
  if (active.length > 1) active.sort((a, b) => Number(b.endsAt || 0) - Number(a.endsAt || 0));
  state.active = active.slice(0, 1);
  saveRaw(state);
  return state;
}
...
function activate(id, options = {}) {
  ...
  if (current && current.id !== id && !options.force) {
    const ok = window.confirm(`${current.name}이(가) 활성화 중입니다.\n${def.name}으로 교체하면 기존 효과가 종료됩니다.\n교체하시겠습니까?`);
    if (!ok) return { ok: false, reason: 'cancelled', conflict: current };
    state.active = [];
  }
  const existing = state.active.find((item) => item.id === id);
  if (existing) existing.endsAt = Math.max(existing.endsAt, now()) + durationMs;
  else state.active = [{ id, name: def.name, icon: def.icon, desc: def.desc, startedAt: now(), endsAt: now() + durationMs }];
  saveRaw(state);
  toast(`${def.icon} ${def.name} 활성화`);
  addLog(`${def.name} 사용: ${def.desc}`, def.icon);
  setTimeout(() => window.CATCHABUGS_GAME?.spawnBoosted?.(def.spawnNow || 1), 80);
...
function activeHTML() {
  ...
  const item = active[0];
  return `<div class="activeItems compact"><div class="activePill"><b>${item.icon} ${item.name}</b><span>${formatRemaining(remainingMs(item))}</span></div></div>`;
}
function injectHud() {
...
function injectStyle() {
  ...
  style.textContent = `#activeItemsHud{position:fixed;left:10px;top:82px;z-index:18;pointer-events:none;max-width:190px}.activeItems{display:flex;flex-direction:column;gap:5px}.activeItems.empty{display:none}.activePill{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 9px;border-radius:999px;background:#07111edc;color:white;box-shadow:0 8px 18px #0004;font-size:11px;font-weight:1000}.activePill span{color:#82f7c1}`;
...
function init() {
  injectStyle();
  injectHud();
  setInterval(() => { cleanState(); injectHud(); pulseRareAlarm(); }, 1500);
}
