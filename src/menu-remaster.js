...
const RETURN_POINTS = Object.freeze([
  { id: 'lab', icon: '🏠', name: '연구소', desc: '연구 업그레이드와 장비 개선을 진행합니다. 실제 이동 지점은 아닙니다.', type: 'lab', defaultOpen: true },
...
function loadCore() { return safeParse(localStorage.getItem(CORE_SAVE_KEY)) || { points: 0, caught: {}, player: { x: 0, y: 0 }, lastEvent: '' }; }
...
function loadSettings() { return safeParse(localStorage.getItem(MENU_SETTINGS_KEY)) || { sound: true, vibration: true, developerMode: false, returnAllOpen: false, infiniteMoney: false, noteAllOpen: false }; }
...
function wireNavButtons() { const close = $('#closeModal'); const back = $('[data-menu-back]'); const exit = $('[data-menu-close]'); if (close) close.onclick = goBack; if (back) back.onclick = goBack; if (exit) exit.onclick = closeModal; }
...
function triggerTarget(id) { const target = document.getElementById(id); if (!target) { toast('아직 준비 중인 메뉴입니다.'); return; } const modal = $('#modal'); if (modal) modal.style.display = 'none'; setTimeout(() => target.click(), 20); }
...
function route(panel, options = {}) { if (!panel) return; const push = options.fromBack ? {} : { push: panel }; if (panel.type === 'hub') openHub(panel.id, push); if (panel.type === 'noteDex') openNoteDex(push); if (panel.type === 'noteExplore') openNoteExplore(push); if (panel.type === 'settings') openSettings(push); if (panel.type === 'developer') openDeveloper(push); }
...
function openNoteHub(options = {}) {
  openModal(`${headerHTML('📖 연구노트', '기록 2분류', '도감기록과 탐험기록만 남겨 피로도를 줄였습니다.')}
    <div class="menuHubGrid">
      ${card('📘', '도감기록', '곤충도감, 도감보상, 전설곤충 도감을 확인합니다.', 'data-panel="noteDex"')}
      ${card('📒', '탐험기록', '탐험일지, 호박사의 메모, 발견한 팁, 지역정보, 세계관을 확인합니다.', 'data-panel="noteExplore"')}
...
function openNoteDex(options = {}) {
  openModal(`${headerHTML('📘 도감기록', '도감 / 전설', '일반 곤충 도감과 전설 곤충 기록을 분리해 확인합니다.')}
    <div class="menuHubGrid">
      ${card('🐞', '곤충도감', '일반 곤충 기록과 최고 등급, 판정을 확인합니다.', 'data-target="openDex"')}
      ${card('🎁', '도감보상', '도감 완성도 보상을 수령합니다.', 'data-target="openDexReward"')}
      ${card('⚡', '전설곤충 도감', '전설 곤충은 포획 대상이 아닌 별도 기록 대상입니다.', 'data-target="openLegendary"')}
...
function openNoteExplore(options = {}) {
  ...
  openModal(`${headerHTML('📒 탐험기록', '기록 모음', '팁과 세계관은 메뉴가 아니라 탐험 중 축적되는 기록으로 관리합니다.')}
    ...
    <div class="compactList"><b>호박사의 메모</b><p>레이더 신호는 곤충, 전설 곤충, 거점 발견으로 확장됩니다. 가까이 갈수록 정체가 드러나는 방향으로 리마스터합니다.</p></div>
    <div class="compactList"><b>발견한 팁</b><p>비 오는 날에는 강가 신호, 밤에는 발광 신호를 우선 확인하세요. 채집 실패 시 곤충 도주 규칙이 다음 밸런스 리마스터에 반영됩니다.</p></div>
    <div class="compactList"><b>지역정보</b><p>숲, 초원, 강가, 도시는 각기 다른 곤충과 거점을 숨기고 있습니다. 거점은 레이더에 표시되지 않고 직접 발견해야 열립니다.</p></div>
...
function openQuestHub(options = {}) {
  openModal(`${headerHTML('📜 퀘스트', '목표 / 업적', '퀘스트는 탭을 나누지 않고 한 화면에서 스크롤로 확인하도록 정리했습니다.')}
    <div class="menuHubGrid">
      ${card('📜', '퀘스트', '메인 퀘스트, 서브 퀘스트, 일일미션을 한 화면에서 확인합니다.', 'data-target="openQuest"')}
...
function openReturnHub(options = {}) {
  ...
  openModal(`${headerHTML('🏕 귀환', '귀환석 / 거점', '연구소는 연구·장비 메뉴이며, 마을과 발견한 거점은 귀환석 이동 지점입니다.')}
...
function openSettings(options = {}) {
  openModal(`${headerHTML('⚙️ 설정', '저장 / 환경 / 개발', '저장, 사운드, 진동, 개발자모드, 게임정보를 이곳으로 모았습니다.')}
    <div class="menuHubGrid">
      ...
      ${card('🧪', '개발자모드', '거점 전체오픈, 연구별 무한, 날씨/시간 변경을 설정합니다.', 'data-panel="developer"')}
...
function devWeatherButton(id, label) { return `<button data-dev-weather="${id}">${label}</button>`; }
function devPhaseButton(id, label) { return `<button data-dev-phase="${id}">${label}</button>`; }
function openDeveloper(options = {}) {
  ...
  openModal(`${headerHTML('🧪 개발자모드', '테스트', '일반 플레이에서는 숨겨진 테스트용 기능입니다.')}
    ...
    <div class="devPanel"><h3>날씨/시간 변경</h3><div class="devActions">
      ${devWeatherButton('clear', '☀️ 맑음')}${devWeatherButton('cloudy', '☁️ 흐림')}${devWeatherButton('rain', '🌧️ 비')}${devWeatherButton('wind', '🍃 바람')}${devWeatherButton('night-glow', '🌙 밤빛')}
      ${devPhaseButton('아침', '아침')}${devPhaseButton('낮', '낮')}${devPhaseButton('저녁', '저녁')}${devPhaseButton('밤', '밤')}
    </div></div>
    <div class="devPanel"><h3>테스트 바로가기</h3><div class="devActions"><button data-target="openLegendary">전설 도감</button><button data-target="openWeather">날씨 정보</button></div></div>${closeHTML()}`, { push: options.push || { type: 'developer' } });
  document.querySelectorAll('[data-dev]').forEach((button) => { button.onclick = () => { const next = loadSettings(); next[button.dataset.dev] = !next[button.dataset.dev]; saveSettings(next); toast(`${button.textContent} ${next[button.dataset.dev] ? 'ON' : 'OFF'}`); openDeveloper({ push: { type: 'developer' } }); }; });
  document.querySelectorAll('[data-target]').forEach((button) => { button.onclick = () => triggerTarget(button.dataset.target); });
  document.querySelectorAll('[data-dev-weather]').forEach((button) => { button.onclick = () => { const api = window.CATCHABUGS_TIME_WEATHER; if (!api?.setWeather) { toast('날씨 시스템 준비 중'); return; } api.setWeather(button.dataset.devWeather); }; });
  document.querySelectorAll('[data-dev-phase]').forEach((button) => { button.onclick = () => { const api = window.CATCHABUGS_TIME_WEATHER; if (!api?.setPhase) { toast('시간 시스템 준비 중'); return; } api.setPhase(button.dataset.devPhase); }; });
...
function openGameInfo() {
  openModal(`${headerHTML('ℹ️ 게임정보', 'Engine 2.1', '보정 모듈을 줄이고 메뉴 본체에서 직접 관리합니다.')}
    <div class="compactList"><b>하단 메뉴</b><p>연구노트 / 퀘스트 / 귀환 / 설정 4개로 고정합니다.</p></div>
    <div class="compactList"><b>연구노트</b><p>일반 도감, 전설 도감, 탐험기록 중심으로 정리했습니다.</p></div>
...
function initMenuRemaster() { buildHub(); const bottom = $('.bottom'); if (!bottom || bottom.dataset.menuObserver === 'on') return; bottom.dataset.menuObserver = 'on'; const observer = new MutationObserver(() => buildHub()); observer.observe(bottom, { childList: true, subtree: false }); }
