...
const BADGES = Object.freeze([
  { id: 'rookie-researcher', icon: '🔰', badge: '초보 연구 배지', title: '새내기 연구원', desc: '곤충을 처음 채집하면 획득한다.', check: (stats) => (stats.catchTotal || 0) >= 1 },
  { id: 'field-collector', icon: '🧺', badge: '채집가 배지', title: '초보 채집가', desc: '곤충을 10마리 이상 채집하면 획득한다.', check: (stats) => (stats.catchTotal || 0) >= 10 },
  { id: 'precision-hand', icon: '🎯', badge: '정밀 채집 배지', title: '정밀한 손놀림', desc: 'PERFECT 판정 3회 이상 달성하면 획득한다.', check: (stats) => (stats.perfectCatch || 0) >= 3 },
  { id: 'rare-tracker', icon: '💎', badge: '희귀 신호 배지', title: '희귀 신호 추적자', desc: '보라 등급 이상 곤충을 3마리 이상 채집하면 획득한다.', check: (stats) => (stats.rareCatch || 0) >= 3 },
  { id: 'dex-scholar', icon: '📘', badge: '도감학자 배지', title: '도감 수집가', desc: '서로 다른 곤충 5종을 기록하면 획득한다.', check: (stats) => (stats.uniqueSpecies || []).length >= 5 },
  { id: 'butterfly-watcher', icon: '🦋', badge: '나비 관찰 배지', title: '나비 관찰자', desc: '나비류를 3마리 이상 채집하면 획득한다.', check: (stats) => Number(stats.familyCatch?.['나비'] || 0) >= 3 },
  { id: 'river-observer', icon: '🌊', badge: '강가 조사 배지', title: '강가 조사원', desc: '강가 서식 곤충을 3마리 이상 채집하면 획득한다.', check: (stats) => Number(stats.habitatCatch?.river || 0) >= 3 },
  { id: 'gold-specimen', icon: '🏆', badge: '황금 표본 배지', title: '황금 표본 발견자', desc: '골드 등급 이상 곤충을 채집하면 획득한다.', check: (stats) => Number(stats.gradePts?.['110'] || 0) >= 1 },
]);

function $(selector) { return document.querySelector(selector); }
function safeParse(raw) { try { return raw ? JSON.parse(raw) : null; } catch { return null; } }
function loadAchievementStats() { const saved = safeParse(localStorage.getItem(ACHIEVEMENT_STORAGE_KEY)); return saved?.stats || {}; }
function loadBadgeState() { return safeParse(localStorage.getItem(BADGE_STORAGE_KEY)) || { equippedTitle: null, equippedBadge: null }; }
function saveBadgeState(state) { localStorage.setItem(BADGE_STORAGE_KEY, JSON.stringify(state)); }
function unlockedBadges() { const stats = loadAchievementStats(); return BADGES.map((badge) => ({ ...badge, unlocked: badge.check(stats) })); }
...
function removeFloatingTitle() { const old = $('#titleBadgeCard'); if (old) old.remove(); }
function updateEquippedDisplay() { removeFloatingTitle(); syncProfileTitle(); }
function syncProfileTitle() {
  const state = loadBadgeState();
  if (!state.equippedTitle) return;
  const profileApi = window.CATCHABUGS_PROFILE;
  if (profileApi?.grantCustomization) profileApi.grantCustomization('title', state.equippedBadge || state.equippedTitle);
  if (profileApi?.equipCustomization) profileApi.equipCustomization('title', state.equippedTitle);
  profileApi?.updateHud?.();
}
...
function initBadgeTitleSystem() { ensureBadgeButton(); updateEquippedDisplay(); }
