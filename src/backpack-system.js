...
const DEFAULT_ITEMS = {
  consumable: [
    { id: 'bug-cookie', icon: '🍪', name: '곤충 쿠키', desc: '15분 동안 일반 곤충 출현이 조금 증가한다.', count: 0 },
  ],
  installation: [
    { id: 'bug-lure', icon: '🪤', name: '벌레 유인기', desc: '30분 동안 주변 곤충 생성량을 높인다.', count: 0 },
    { id: 'rare-alarm', icon: '🔔', name: '희귀 알람기', desc: '30분 동안 희귀 등급 이상 신호를 알려준다.', count: 0 },
    { id: 'radar-boost', icon: '📡', name: '레이더 증폭기', desc: '30분 동안 레이더 탐지 범위를 넓힌다.', count: 0 },
    { id: 'sap-trap', icon: '🪵', name: '수액통', desc: '45분 동안 숲 곤충 유인 효과가 증가한다.', count: 0 },
    { id: 'flower-lure', icon: '🌼', name: '꽃 유인기', desc: '45분 동안 초원 곤충 유인 효과가 증가한다.', count: 0 },
    { id: 'water-trap', icon: '💧', name: '수생 트랩', desc: '45분 동안 강가 수서 곤충 유인 효과가 증가한다.', count: 0 },
    { id: 'bug-light', icon: '💡', name: '곤충등', desc: '45분 동안 야행성 곤충 신호를 강화한다.', count: 0 },
  ],
  gear: [
...
function activeItems() { return window.CATCHABUGS_ACTIVE_ITEMS || null; }
...
function activeLine() {
  const active = activeItems()?.getActiveItems?.() || [];
  if (!active.length) return '<div class="bagActive empty">활성 설치물 없음</div>';
  return `<div class="bagActive">${active.map((item) => `<span>${item.icon} ${item.name} ${activeItems()?.formatRemaining?.(activeItems()?.remainingMs?.(item)) || ''}</span>`).join('')}</div>`;
}
function itemCard(item, action = '') {
  const active = activeItems()?.isActive?.(item.id);
  const status = 'count' in item ? `보유 ${item.count}${active ? ' · 활성중' : ''}` : item.owned ? '보유중' : '미보유';
  return `<article class="bagCard ${active ? 'active' : ''}"><div class="bagIcon">${item.icon}</div><div class="bagInfo"><b>${item.name}</b><p>${item.desc}</p><small>${status}</small></div>${action}</article>`;
}
...
function backpackHTML(tab = 'installation') {
  ...
  return `<div class="bagSheet"><h2>🎒 배낭</h2>${currencyLine()}${activeLine()}<div class="bagTabs">${tabs}</div><div class="bagList">${renderSection(tab)}</div></div>
    <style>
      .bagSheet h2{margin:0 0 8px}.bagCurrency,.bagActive{padding:10px;border-radius:16px;background:#0000000a;font-size:12px;font-weight:1000;color:#0f6f56;margin-bottom:7px}.bagActive{display:flex;flex-wrap:wrap;gap:6px;color:#07111e}.bagActive.empty{color:#0007}.bagActive span{border-radius:999px;background:#fff;padding:6px 8px}.bagTabs{display:flex;gap:6px;margin:10px 0;overflow:auto}.bagTabs button{border:0;border-radius:999px;padding:8px 12px;font-weight:1000;background:#0000000d;color:#07111e;white-space:nowrap}.bagTabs button.on{background:#07111e;color:white}.bagList{display:grid;gap:9px}.bagCard{display:flex;align-items:center;gap:10px;border-radius:20px;background:linear-gradient(135deg,#fff,#f6fbff);border:1px solid #0000000d;padding:11px;box-shadow:0 8px 18px #0001}.bagCard.active{border-color:#82f7c1aa;box-shadow:0 8px 22px #82f7c122}.bagIcon{width:50px;height:50px;flex:0 0 50px;border-radius:17px;background:#fff;display:flex;align-items:center;justify-content:center;font-size:28px;box-shadow:inset 0 0 0 1px #0001}.bagInfo{flex:1}.bagInfo b{font-size:14px}.bagInfo p{margin:4px 0;color:#0009;font-size:12px;font-weight:800;line-height:1.35}.bagInfo small{font-size:11px;font-weight:1000;color:#0f6f56}.bagCard button{border:0;border-radius:12px;background:#07111e;color:white;padding:9px 10px;font-weight:1000}.bagCard button:disabled{opacity:.35}
    </style>`;
}
...
function consumeItem(id, tab) {
  const data = loadBackpack();
  const section = data[tab] || [];
  const item = section.find((entry) => entry.id === id);
  if (!item || Number(item.count || 0) <= 0) return false;
...
function wireBackpack() {
  ...
  document.querySelectorAll('[data-use-item]').forEach((button) => {
    button.onclick = () => {
      ...
      const item = consumeItem(id, tab);
      if (!item) { toast('보유 수량이 부족합니다.'); return; }
      const result = activeItems()?.activate?.(id);
      if (!result?.ok) { addItem(id, 1); toast('아직 사용할 수 없는 아이템입니다.'); return; }
